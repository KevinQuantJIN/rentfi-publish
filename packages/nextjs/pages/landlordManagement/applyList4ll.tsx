...
const ApplyList4ll = () => {
  ...
  const { setSelectedApply4ll } = useContext(UserTypeContext);

  const handleApplyClick = (s: APPLY) => {
    setSelectedApply4ll(s);
  };
  const handleSearch = async () => {
    // console.log(address);
    try {
      const response = await fetch(`/api/applys4ll?landlord=${address}`, {
        method: "GET",
        headers: {
  ...
  return (
    <div className="w-full p-5">
      <h1 className="mb-5 text-6xl font-bold font-sans text-black">Apply Record List</h1>
      <div
            ...
            >
              <div className="card-body">
                <div className="tooltip tooltip-open tooltip-right" data-tip={`STATUS:` + s.status}>
                  ...
                  <p className="card-title text-left text-xl">Tenant:</p>
                  <p className="text-left break-all">{s.tenantAddress}</p>
                </div>
